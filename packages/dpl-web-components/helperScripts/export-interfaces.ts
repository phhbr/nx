...
const TYPES_DIR: string = path.join(INTERFACES_DIR, 'types');
...
const OUTPUT_TYPES_INDEX: string = path.join(TYPES_DIR, 'index.ts');
...
function generateTypesIndex(): string[] {
  const files: string[] = getTypeScriptFiles(TYPES_DIR);
  if (!files.length) {
    // If index exists but no files remain, remove it to avoid stale exports
    if (fs.existsSync(OUTPUT_TYPES_INDEX)) fs.unlinkSync(OUTPUT_TYPES_INDEX);
    return [];
  }
  ...
  const content: string = lines.join('\n') + '\n';
  const changed: boolean = writeIfChanged(OUTPUT_TYPES_INDEX, content);
  if (changed) console.log(`✓ Wrote ${OUTPUT_TYPES_INDEX}`);
  else console.log(`✓ ${OUTPUT_TYPES_INDEX} up-to-date`);
  return files;
}
...
function generateRootIndex(eventFiles: string[]): void {
  ...
  for (const file of files) {
    // skip collisions (there shouldn't be files named 'events.ts' or 'types.ts')
    if (file === 'events' || file === 'types') continue;
    const name = path.basename(file, '.ts');
  ...
  // Check if types directory has exports
  const typesFiles = getTypeScriptFiles(TYPES_DIR);
  if (typesFiles && typesFiles.length) {
...
async function main(): Promise<void> {
  try {
    ...
    // ensure subdirs exist (it's ok if they don't)
